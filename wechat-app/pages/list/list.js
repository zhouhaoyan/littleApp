...
Page({
  ...
  onReachBottom: function () {
    ...
    var that = this;
    wx.request({
      url: 'http://47.104.142.33:8080/album/page/classify?pageSize=' + that.data.pageSize + '&pageNo=' + (that.data.pageNo+1) + '&classifyId=' + that.data.classifyId,
  ...
  getImgList: function () {
    var that = this;
    wx.request({
      url: 'http://47.104.142.33:8080/album/page/classify?pageSize=' + that.data.pageSize + '&pageNo=' + that.data.pageNo + '&classifyId=' + that.data.classifyId,
      success: function (res) {
        console.log(res.data.data.list);
        that.setData({
          imgListKey: that.data.imgListKey.concat(res.data.data.list),
